...
class Menu_Screen:
    ...
    def draw(self):
        ...
        if self.start_button.click():
            game_mode_menu = Game_Mode_Menu(self.screen)
            return game_mode_menu.run()
...
class Game_Mode_Menu:
    def __init__(self, screen):
        ...
        # Load images for buttons
        single_player_img = pygame.image.load("../Images/single_player_image.png").convert_alpha()
        single_player_w, single_player_h = 250, 75
        single_player_img = pygame.transform.scale(single_player_img, (single_player_w, single_player_h))

        multiplayer_img = pygame.image.load("../Images/multiplayer_image.png").convert_alpha()
        multiplayer_w, multiplayer_h = 250, 75
        multiplayer_img = pygame.transform.scale(multiplayer_img, (multiplayer_w, multiplayer_h))

        self.single_player_button = Button(screen, (screen.get_width() // 2) - (single_player_w // 2), (screen.get_height() // 3.5) - (single_player_h // 2), single_player_img)
        self.multiplayer_button = Button(screen, (screen.get_width() // 2) - (multiplayer_w // 2), (screen.get_height() // 2) - (multiplayer_h // 1.5), multiplayer_img)
    ...
    def draw(self):
        ...
        if self.single_player_button.click():
            # Initialize single-player game here
            print("Single player game selected")
            # Code to initialize single-player game goes here
            return "single_player"
        if self.multiplayer_button.click():
            return "multiplayer"
